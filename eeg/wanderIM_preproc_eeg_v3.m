...
for n=1:length(files)
    ...
    %%%%% Epoch baseline
    if exist([preproc_path filesep 'basel_' D.fname])==0 || redo.epoching==1
        pretrig  =  -5 * D.fsample;
    ...
    %%%%% Epoch probes
    if exist([preproc_path filesep 'probe_' D.fname])==0 || redo.epoching==1
    ...
    %%%%% Epoch trials
    if exist([preproc_path filesep 'trial_' D.fname])==0 || redo.epoching==1
    ...
    if exist([preproc_path filesep epochFilename(1:end-4) '_ica.set'])==0 || redo.ica==1
        paramICA=[];
