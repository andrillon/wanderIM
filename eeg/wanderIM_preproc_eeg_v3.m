...
for n=length(files)
    ...
    if exist([preproc_path filesep 'trial_' D.fname])==2 && exist([preproc_path filesep 'basel_' D.fname])==2 && exist([preproc_path filesep 'probe_' D.fname])==2 && sum([redo.epoch.bsl redo.epoch.trial redo.epoch.probe redo.epoch.hb])==0
    else
        %%% Find Triggers
        clear din_*
        ...
        if strcmp(subID,'307') %307 has two blocks in excess
            blockidx=match_str({myevents.type},'B');
            myevents(1:blockidx(3)-1)=[];
            warning('... correction for 307!')
            fprintf('... ... %g blocks found (end %g)\n',length(match_str({myevents.type},'B')),length(match_str({myevents.type},'K')));
            fprintf('... ... %g probes found (end %g)\n',length(match_str({myevents.type},'P')),length(match_str({myevents.type},'C')));
            fprintf('... ... %g trials found \n',length(match_str({myevents.type},'T')));
        end
        ...
        %%%%% Epoch trials
        if exist([preproc_path filesep 'trial2_' D.fname])==0 || redo.epoch.trial==1
