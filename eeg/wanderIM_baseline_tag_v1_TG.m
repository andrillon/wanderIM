...
%     % Apply Laplacian
%     if n==1
%         myLabels=D.chanlabels;
%     [M] = ExtractMontage ([path_CSD filesep 'resource' filesep '10-5-System_Mastoids_EGI129.csd'], myLabels(1:63)');
%     [G,H] = GetGH(M);
%     end
%     lapD=nan([63,size(D,2),size(D,3)]);
%     for ntr=1:D.ntrials
%         lapD(:,:,ntr) = CSD (D(1:63,:,ntr), G, H);
%     end
