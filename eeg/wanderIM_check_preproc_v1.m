...
for n=1:length(left_freq)
    [~,this_f]=findclosest(faxis,left_freq(n)/2);
    temp_topo(n,:)=squeeze(mean(mean(mean(baseline_logSNR(n,:,:,:,this_f),1),2),3))';
end
...
temp_topo=[];
for n=1:length(left_freq)
    [~,this_f]=findclosest(faxis,right_freq(n)/2);
    temp_topo(n,:)=squeeze(mean(mean(mean(baseline_logSNR(n,:,:,:,this_f),1),2),3))';
end
simpleTopoPlot2(mean(temp_topo(:,1:63),1), pos', labels,0,[],0,lay,[]);
caxis([-1 1])
...
temp_topo=[];
for n=1:length(left_freq)
    [~,this_f]=findclosest(faxis,left_freq(n));
    temp_topo(n,:)=squeeze(mean(mean(mean(baseline_logSNR(n,:,:,:,this_f),1),2),3))';
end
simpleTopoPlot2(mean(temp_topo(:,1:63),1), pos', labels,0,[],0,lay,[]);
caxis([-1 1])
...
temp_topo=[];
for n=1:length(left_freq)
    [~,this_f]=findclosest(faxis,right_freq(n));
    temp_topo(n,:)=squeeze(mean(mean(mean(baseline_logSNR(n,:,:,:,this_f),1),2),3))';
end
simpleTopoPlot2(mean(temp_topo(:,1:63),1), pos', labels,0,[],0,lay,[]);
caxis([-1 1])
...
[~,this_f]=findclosest(faxis,10.5);
temp_topo=[];
temp_topo=squeeze(mean(mean(mean(baseline_logSNR(:,:,:,:,this_f),1),2),3));
simpleTopoPlot2(temp_topo, pos', labels,0,[],0,lay,[]);
caxis([-1 1]*0.5)
title('IM 10.5')

subplot(3,2,6); format_fig; % IM (13.5 Hz)
[~,this_f]=findclosest(faxis,13.5);
temp_topo=[];
temp_topo=squeeze(mean(mean(mean(baseline_logSNR(:,:,:,:,this_f),1),2),3));
simpleTopoPlot2(temp_topo, pos', labels,0,[],0,lay,[]);
caxis([-1 1]*0.5)
