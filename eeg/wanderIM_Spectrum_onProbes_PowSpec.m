...
subplot(2,1,1)
for nstate=1:3
    temp=mean(all_probes_mat2(all_probes_mat2(:,7)==nstate & all_probes_mat2(:,21)==match_str(D.chanlabels,'Fz'),22:end));
...
clear Alpha_*
for nE=1:63
    subtbl_probe=tbl_probe(tbl_probe.Chan==nE & tbl_probe.Task=="1",:);
    mdl_Alpha_perE= fitlme(subtbl_probe,'alpha~nBlock+State+(1| SubID)');
    Alpha_F_tval(:,nE)=double(mdl_Alpha_perE.Coefficients(:,4));
    Alpha_F_pval(:,nE)=double(mdl_Alpha_perE.Coefficients(:,6));
    EffectNames=mdl_Alpha_perE.CoefficientNames;
    
    subtbl_probe=tbl_probe(tbl_probe.Chan==nE & tbl_probe.Task=="2",:);
    mdl_Alpha_perE= fitlme(subtbl_probe,'alpha~nBlock+State+(1| SubID)');
    Alpha_D_tval(:,nE)=double(mdl_Alpha_perE.Coefficients(:,4));
    Alpha_D_pval(:,nE)=double(mdl_Alpha_perE.Coefficients(:,6));
...
figure;
addpath(genpath('/Users/tand0009/Work/local/eeglab14_1_2b/'));
varNames={'Task_2','State_3'};
for nplot=1:length(varNames)
    subplot(2,length(varNames),nplot)
    tempplot=Alpha_D_tval(match_str(EffectNames,varNames{nplot}),:);
    tempplotpV=Alpha_D_pval(match_str(EffectNames,varNames{nplot}),:);
    % tempplot(tempplotpV>0.005)=0;
    if ~isempty(fdr(tempplotpV,0.05))
    topoplot(tempplot, layout.chaninfo,'style','map','electrodes','off','emarker2',{find(tempplotpV<fdr(tempplotpV,0.05)),'.','k',10,2});
    else
    topoplot(tempplot, layout.chaninfo,'style','map','electrodes','off');
    end
...
%     
    ...
    subplot(2,length(varNames),nplot+2)
    tempplot=Alpha_F_tval(match_str(EffectNames,varNames{nplot}),:);
    tempplotpV=Alpha_F_pval(match_str(EffectNames,varNames{nplot}),:);
    % tempplot(tempplotpV>0.005)=0;
        if ~isempty(fdr(tempplotpV,0.05))
topoplot(tempplot, layout.chaninfo,'style','map','electrodes','off','emarker2',{find(tempplotpV<fdr(tempplotpV,0.05)),'.','k',10,2});
        else
topoplot(tempplot, layout.chaninfo,'style','map','electrodes','off');
        end%     topoplot(find(tempplotpV<0.05), layout.chaninfo,'style','blank','electrodes','on','emarker',{'.','k',20,2});
%     topoplot(find(tempplotpV<fdr(tempplotpV,0.05)), layout.chaninfo,'electrodes','on','style','blank','emarker',{'o','k',20,2});
%       
    caxis([-1 1]*4)
    title(varNames{nplot})
