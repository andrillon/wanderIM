...
all_amp_Waves=[];
erp_all=[];
for n=[1:4 6:20] % for each participant (generate a topography of correlation coefficients)
...
format_fig;
addpath(genpath(path_eeglab));
temp_topo=mean(maxnegamp_all,2); % vector of 63 values
topoplot(temp_topo, layout.chaninfo,'style','map','whitebk','on','electrodes','on');
cmap=colormap('parula'); %cmap=flipud(cmap); colormap(cmap);
% caxis([0 2.5])
rmpath(genpath(path_eeglab));
colorbar;
caxis([-25 max(temp_topo)])
...
for n=1:length(bsl_files)
    ...
    for nE = 1:63
        ...
        thr_wave = prctile(all_Waves(all_Waves(:,3)==nE,2),80);
        a = zeros(60,1);
        for nPr = 1:60
