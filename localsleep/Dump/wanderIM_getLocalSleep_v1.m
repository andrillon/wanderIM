...
%     right_freq(n)=SubjectInfo.FlickerL;
    ...
    for npr=1:size(temp_data,3)
        ...
        [twa_results filtered_data]=twalldetectnew_TA(datainput,D.fsample,0,npr,filtered_data);
     ...
     save([eeg_path filesep 'fwanderIM_twa2_' SubID],'all_Waves')
%     save([eeg_path filesep 'EEGwanderIM_twa2_' SubID],'filtered_data')
    for nE=1:63
        thr_Wave1(n,nE)=prctile(all_Waves(all_Waves(:,3)==nE,4),80);
        thr_Wave2(n,nE)=prctile(all_Waves(all_Waves(:,3)==nE,4),90);
    end
%     save([eeg_path filesep 'thrwave_wanderIM_twa2_' SubID],'thr_Wave1')
