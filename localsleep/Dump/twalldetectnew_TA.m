function [twa_results filtered_data]=twalldetectnew_TA(datainput,orig_fs,thramp,nPr,filtered_data)
...
Wp=[1.0 10.0]/(fs/2); % Filtering parameters 4.5
Ws=[0.1 15]/(fs/2); % Filtering parameters 2
Rp=3;
Rs=25;
...
disp([' Analyzing ', num2str(size(datainput,1)),' channels']);
% h = waitbar(0,'Detection Progress');

abc = zeros(63, 2560);
for i=1:63 % (i) is the number of the channel
LoadedEEG.data=datainput ;
...
allavg=nanmean(abc);
dataref=abc-repmat(allavg,[63 1]);
...
% 	signal = resample(double(datax),fs,orig_fs);
%     EEG=filtfilt(bbp, abp, signal);
%     filtered_data(i,1:2560,nPr)=EEG;
    EEG = dataref(i,:);
