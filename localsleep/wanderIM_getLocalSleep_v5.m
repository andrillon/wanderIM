...
%     these_times=D.indsample(-32):D.indsample(0)-1;
    ...
    for npr=1:size(temp_data,3)
        ...
        [twa_results]=twalldetectnew_TA_v2(datainput,D.fsample,0);
%         save([eeg_path filesep 'wanderIM_twa5_noica_raw_' SubID '_P' num2str(npr)],'twa_results')
        for nE=1:63
            all_Waves=[all_Waves ; [repmat([n npr nE],length(abs(cell2mat(twa_results.channels(nE).maxnegpkamp))),1) abs(cell2mat(twa_results.channels(nE).maxnegpkamp))'+abs(cell2mat(twa_results.channels(nE).maxpospkamp))' ...
                cell2mat(twa_results.channels(nE).negzx)'+D.time(1)*D.fsample ...
                cell2mat(twa_results.channels(nE).poszx)'+D.time(1)*D.fsample ...
                cell2mat(twa_results.channels(nE).wvend)'+D.time(1)*D.fsample ...
                cell2mat(twa_results.channels(nE).maxnegpk)'+D.time(1)*D.fsample ...
                cell2mat(twa_results.channels(nE).maxnegpkamp)' ...
                cell2mat(twa_results.channels(nE).maxpospk)'+D.time(1)*D.fsample ...
    ...
    save([eeg_path filesep 'wanderIM_twa5_noica_bigwnd_' SubID],'all_Waves')
%     for nE=1:63
