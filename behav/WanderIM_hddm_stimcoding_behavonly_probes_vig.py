...
data = hddm.load_csv('/Users/tand0009/Data/WanderIM/behav/WanderIM_ProbeResults2_thWaves_vig.txt')
data.columns = ['subj_idx','nblock','nprobe','task','ntrial','look','state','orig','awa','int','eng','perf','vig','corr','rt','stim','dprobe','nwave','response','stimulus','vigC','cond_v']
...
v_g1, v_g2, v_ng1, v_ng2 = modelD.nodes_db.node[['v(go_w-)', 'v(go_w+)','v(nogo_w-)', 'v(nogo_w+)']]
hddm.analyze.plot_posterior_nodes([v_g1, v_g2, v_ng1, v_ng2])
plt.xlabel('Drift')
plt.ylabel('Posterior probability')
plt.title('Posterior of Drift between timepoints')

print("P(v(go_w-) > v(go_w+)) = ", (v_g1.trace() > v_g2.trace()).mean())
print("P(v(nogo_w-) > v(nogo_w+)) = ", (v_ng1.trace() > v_ng2.trace()).mean())
...
means = [mean_v_g1, mean_v_g0, mean_v_ng1, mean_v_ng0, mean_z_1, mean_z_0, mean_a_1, mean_a_0, mean_t_1, mean_t_0]
errors = [std_v_g1, std_v_g0, std_v_ng1, std_v_ng0, std_z_1, std_z_0, std_a_1, std_a_0, std_t_1, std_t_0]
...
plt.tight_layout()
plt.savefig('figures/HDDM_localsleep_v.png')
...
plt.tight_layout()
plt.savefig('figures/HDDM_localsleep_zat.png')
