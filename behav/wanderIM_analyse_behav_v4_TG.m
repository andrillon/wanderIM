...
%% Performance results by task
mean(squeeze(pption_MS(:,1,1)))
mean(squeeze(pption_MS(:,1,2)))
mean(squeeze(pption_MS(:,1,3)))

mean(squeeze(pption_MS(:,2,1)))
mean(squeeze(pption_MS(:,2,2)))
mean(squeeze(pption_MS(:,2,3)))

sem(squeeze(pption_MS(:,1,1)))
sem(squeeze(pption_MS(:,1,2)))
sem(squeeze(pption_MS(:,1,3)))

sem(squeeze(pption_MS(:,2,1)))
sem(squeeze(pption_MS(:,2,2)))
sem(squeeze(pption_MS(:,2,3)))

%% Origin of Mind Wandering Results 2nd dimension = task, 3rd dimension = origin where 1='room',2='perso',3='task'
sem(squeeze(pption_Ori(:,1,1)))
sem(squeeze(pption_Ori(:,1,2)))
sem(squeeze(pption_Ori(:,1,3)))

sem(squeeze(pption_Ori(:,2,1)))
sem(squeeze(pption_Ori(:,2,2)))
sem(squeeze(pption_Ori(:,2,3)))