...
data_path='/media/tLab_BackUp1/Monash/WanderIM/behav/';
...
for n=1:length(files)
    ...
    % 11: go
    for nbt=1:2
        tp_nogos=test_res(test_res(:,2)==nbt & ~isnan(test_res(:,10)),10);
        tp_gos=test_res(test_res(:,2)==nbt & ~isnan(test_res(:,11)),11);
    ...
    all_test_res=[all_test_res ; [n*ones(size(test_res,1),1) test_res(:,[1 2 3 10 11])]];

    % probe
    %  1: num probe within block
    %  2: probe onset
    %  3: num block
    %  4: block type (1: FACE / 2: SQUARE)
    %  5: num trial
    %  6: resp Q1 Looking (1: Yes / 2: No)
    %  7: resp Q2 Mind-State (1: On / 2: MW / 3: MB / 4: ?)
    %  8: resp Q3 Origine (1: room / 2: personal / 3: task)
    %  9: resp Q4 Awareness (1 (fully) to 4 (not at all))
    % 10: resp Q5 Intention (1 (fully) to 4 (not at all)) 
    % 11: resp Q6 Engagement (1 (not) to 4 (very)) 
    % 12: resp Q7 Performance (1 (bad) to 4 (good)) 
    % 13: resp Q8 Alterness (1 (alert) to 4 (sleepy)) 
    % 14-21: onset R1-8
    % 22-29: onset Q1-8
    % 30-37: resp number Q1-8
    for nbt=1:2
        temp=probe_res(probe_res(:,4)==nbt,[4 30:37]);
    ...
    for nb=1:max(probe_res(:,3))
            temp=probe_res(probe_res(:,3)==nb,[4 30:37]);
            temp2=test_res(test_res(:,1)==nb,[10 11]);
%             pption=[];
%             for nstate=1:4
%                 pption(nstate)=mean(temp(:,3)==nstate);
%             end
%             all_test_resprobes_perblock=[all_test_resprobes_perblock ; [n 1 unique(temp(:,1)) pption mean(temp(:,[5:8])) nanmean(temp2)]];
    ...
    for nbt=1:2
        tp_probes=probe_res(probe_res(:,4)==nbt,30:37);
