...
%     perfNOGO_est{1}=[perfNOGO_est{1} ; [nE double(perfNOGO_mdl2.Coefficients(3,2)) double(perfNOGO_mdl2.Coefficients(3,4)) double(perfNOGO_mdl2.Coefficients(3,6))]];% double(perfNOGO_mdl.Coefficients(4,2)) double(perfNOGO_mdl.Coefficients(4,4)) double(perfNOGO_mdl.Coefficients(4,6))]];
    ...
    [real_out, perm_out]=lme_perm(GO_table,GO_table.Properties.VariableNames{10+nE},'RT~1+pred+Task+(1|SubID)',totperm);
    ...
    [real_out, perm_out]=lme_perm(GO_table,GO_table.Properties.VariableNames{10+nE},'Perf~1+pred+Task+(1|SubID)',totperm);
    ...
    [real_out, perm_out]=lme_perm(NOGO_table,NOGO_table.Properties.VariableNames{10+nE},'Perf~1+pred+Task+(1|SubID)',totperm);
