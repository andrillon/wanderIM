...
recompute=0;
totperm=1;
MWvsON_nWa_est=cell(4,2);
MWvsON_P2P_est=cell(4,2);
MWvsON_DwS_est=cell(4,2);
MWvsON_UpS_est=cell(4,2);

MBvsON_nWa_est=cell(4,2);
MBvsON_P2P_est=cell(4,2);
MBvsON_DwS_est=cell(4,2);
MBvsON_UpS_est=cell(4,2);

MWvsMB_nWa_est=cell(4,2);
MWvsMB_P2P_est=cell(4,2);
MWvsMB_DwS_est=cell(4,2);
MWvsMB_UpS_est=cell(4,2);
...
for nWin=1:4
    ...
    for nE=1:63
        ...
        MWvsON_nWa_est{nWin,1}=[MWvsON_nWa_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MWvsON_P2P_est{nWin,1}=[MWvsON_P2P_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MWvsON_DwS_est{nWin,1}=[MWvsON_DwS_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MWvsON_UpS_est{nWin,1}=[MWvsON_UpS_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MBvsON_nWa_est{nWin,1}=[MBvsON_nWa_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MBvsON_P2P_est{nWin,1}=[MBvsON_P2P_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MBvsON_DwS_est{nWin,1}=[MBvsON_DwS_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MBvsON_UpS_est{nWin,1}=[MBvsON_UpS_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MWvsMB_nWa_est{nWin,1}=[MWvsMB_nWa_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MWvsMB_P2P_est{nWin,1}=[MWvsMB_P2P_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MWvsMB_DwS_est{nWin,1}=[MWvsMB_DwS_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
    tic;
    fprintf('E:%2.0f/63\n',0)
    for nE=1:63
        ...
        MWvsMB_UpS_est{nWin,1}=[MWvsMB_UpS_est{nWin,1} ; [nE real_out]];% double(rtGO_mdl.Coefficients(4,2)) double(rtGO_mdl.Coefficients(4,4)) double(rtGO_mdl.Coefficients(4,6))]];
    end
    toc;
...
all_pVs=[];
for nWin=1:4
    all_pVs=[all_pVs;  MWvsON_nWa_est{nWin,1}(:,4)];
    all_pVs=[all_pVs;  MWvsON_P2P_est{nWin,1}(:,4)];
    all_pVs=[all_pVs;  MWvsON_DwS_est{nWin,1}(:,4)];
    all_pVs=[all_pVs;  MWvsON_UpS_est{nWin,1}(:,4)];
    
    all_pVs=[all_pVs;  MBvsON_nWa_est{nWin,1}(:,4)];
    all_pVs=[all_pVs;  MBvsON_P2P_est{nWin,1}(:,4)];
    all_pVs=[all_pVs;  MBvsON_DwS_est{nWin,1}(:,4)];
    all_pVs=[all_pVs;  MBvsON_UpS_est{nWin,1}(:,4)];
    
    all_pVs=[all_pVs;  MWvsMB_nWa_est{nWin,1}(:,4)];
    all_pVs=[all_pVs;  MWvsMB_P2P_est{nWin,1}(:,4)];
    all_pVs=[all_pVs;  MWvsMB_DwS_est{nWin,1}(:,4)];
    all_pVs=[all_pVs;  MWvsMB_UpS_est{nWin,1}(:,4)];
end
p_FDR=fdr(all_pVs,0.05);
...
for nP=1:4
    myplots(nP)=figure;
    set(myplots(nP),'Position',[5         478        1412         500]);
end
for nP=1:4
    for nWin=1:4
...
%         load(['/Users/tand0009/Data/WanderIM/hddm/HDDM_WIM_thrE90P2P_P2P_Amp_State_LMEandPerm_rev1_MWvsON_UpS_Win' num2str(nWin) '.mat'])
        ...
        subplot(1,4,nWin);
        if nP==1
            temp_topo=MWvsON_nWa_est{nWin,1}(:,3);
            temp_clus=find(MWvsON_nWa_est{nWin,1}(:,4)<p_FDR);
        elseif nP==2
            temp_topo=MWvsON_P2P_est{nWin,1}(:,3);
            temp_clus=find(MWvsON_P2P_est{nWin,1}(:,4)<p_FDR);
        elseif nP==3
            temp_topo=MWvsON_DwS_est{nWin,1}(:,3);
            temp_clus=find(MWvsON_DwS_est{nWin,1}(:,4)<p_FDR);
        elseif nP==4
            temp_topo=MWvsON_UpS_est{nWin,1}(:,3);
            temp_clus=find(MWvsON_UpS_est{nWin,1}(:,4)<p_FDR);
        end
        ...
        if ~isempty(temp_clus)
                ft_plot_lay_me(layout, 'chanindx',temp_clus,'pointsymbol','o','pointcolor','k','pointsize',64,'box','no','label','no')
        end
        title(['MW vs ON: ' Titles{nP}])
%             export_fig([path_fig filesep 'LocalSleep_MWvsON_' Titles{nP} '_Win' num2str(nWin) '.fig'])
        
    end
            export_fig([path_fig filesep 'LocalSleep_MWvsON_' Titles{nP} '_Win.eps'],'-r 300')
end
...
%         load(['/Users/tand0009/Data/WanderIM/hddm/HDDM_WIM_thrE90P2P_P2P_Amp_State_LMEandPerm_rev1_MBvsON_UpS_Win' num2str(nWin) '.mat'])
        ...
        subplot(1,4,nWin);
        if nP==1
            temp_topo=MBvsON_nWa_est{nWin,1}(:,3);
            temp_clus=find(MBvsON_nWa_est{nWin,1}(:,4)<p_FDR);
        elseif nP==2
            temp_topo=MBvsON_P2P_est{nWin,1}(:,3);
            temp_clus=find(MBvsON_P2P_est{nWin,1}(:,4)<p_FDR);
        elseif nP==3
            temp_topo=MBvsON_DwS_est{nWin,1}(:,3);
            temp_clus=find(MBvsON_DwS_est{nWin,1}(:,4)<p_FDR);
        elseif nP==4
            temp_topo=MBvsON_UpS_est{nWin,1}(:,3);
            temp_clus=find(MBvsON_UpS_est{nWin,1}(:,4)<p_FDR);
        end
        ...
        %     export_fig([path_fig filesep 'LocalSleep_MWvsON_' Titles{nP} '.fig'])
            ...
            export_fig([path_fig filesep 'LocalSleep_MBvsON_' Titles{nP} '_Win.eps'],'-r 300')
end
...
for nP=1:4
    for nWin=1:4
        ...
        subplot(1,4,nWin);
        if nP==1
            temp_topo=MWvsMB_nWa_est{nWin,1}(:,3);
            temp_clus=find(MWvsMB_nWa_est{nWin,1}(:,4)<p_FDR);
        elseif nP==2
            temp_topo=MWvsMB_P2P_est{nWin,1}(:,3);
            temp_clus=find(MWvsMB_P2P_est{nWin,1}(:,4)<p_FDR);
        elseif nP==3
            temp_topo=MWvsMB_DwS_est{nWin,1}(:,3);
            temp_clus=find(MWvsMB_DwS_est{nWin,1}(:,4)<p_FDR);
        elseif nP==4
            temp_topo=MWvsMB_UpS_est{nWin,1}(:,3);
            temp_clus=find(MWvsMB_UpS_est{nWin,1}(:,4)<p_FDR);
        end
...
%             export_fig([path_fig filesep 'LocalSleep_MWvsON_' Titles{nP} '.fig'])
            ...
            export_fig([path_fig filesep 'LocalSleep_MWvsMB_' Titles{nP} '_Win.eps'],'-r 300')
end