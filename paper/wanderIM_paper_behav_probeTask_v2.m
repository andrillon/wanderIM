...
for nstate=1:3
    ...
    scatter(1:size(templot,2),nanmean(templot,1),'MarkerEdgeColor',Colors(nstate,:),'MarkerFaceColor','w','Marker','o','SizeData',300,'LineWidth',3);
    hold on;
end
xlabel('Block #');
...
%% Within within block
figure;
format_fig; set(gcf,'Position',[440   171   480   380])
for nstate=1:3
...
% for nS=1:length(myS)
%     for nB=1:6
%         pption_MS2=[pption_MS2 ; [unique(all_probes_mat2(all_probes_mat2(:,1)==myS(nS) & all_probes_mat2(:,2)==nB,3)) mean(all_probes_mat2(all_probes_mat2(:,1)==myS(nS) & all_probes_mat2(:,2)==nB,[4])==1) ...
%             mean(all_probes_mat2(all_probes_mat2(:,1)==myS(nS) & all_probes_mat2(:,2)==nB,[4])==2)...
%             mean(all_probes_mat2(all_probes_mat2(:,1)==myS(nS) & all_probes_mat2(:,2)==nB,[4])==3)]];
%     end
% end
Pairs=[1 2; 1 3; 2 3];
PairColors=[Colors(2,:) ; Colors(3,:) ; [1 1 1]*0.7]; 
PlotMarkers={'d','o'};
figure; set(gcf,'Position',[435   596   971   389]);
for nplot=1:3
    subplot(1,3,nplot)
    for ntask=1:2
        temp1=squeeze(pption_MS(:,ntask,Pairs(nplot,1)));
        temp2=squeeze(pption_MS(:,ntask,Pairs(nplot,2)));
    [~, hdl]=simpleCorPlot(temp1,temp2,{PlotMarkers{ntask},PairColors(nplot,:),PairColors(nplot,:),144},'pearson',0);
%     [~, hdl]=simpleCorPlot(squeeze(pption_MS(:,ntask,Pairs(nplot,1))),squeeze(pption_MS(:,ntask,Pairs(nplot,2))),{PlotMarkers{ntask},Colors(Pairs(nplot,2),:),Colors(Pairs(nplot,1),:),144},'pearson',0);
%     [~, hdl]=simpleCorPlot(pption_MS2(pption_MS2(:,1)==ntask,Pairs(nplot,1)+1),pption_MS2(pption_MS2(:,1)==ntask,Pairs(nplot,2)+1),{PlotMarkers{ntask},PairColors(nplot,:),PairColors(nplot,:),144},'pearson',0);
    xlim([0 1]);
    ylim([0 1]);
    hdl.MarkerFaceAlpha=0.5;
    hdl.LineWidth=2;
    end
end
...
set(gcf,'position',[11   513   375   445])
h   = rm_raincloud(data, Colors(1:2,:),0, 'ks', [],100);
set(gca, 'XLim', [0 1],'YTick','','Ylim',[-3 12]);
% title(['Figure M9' newline 'Repeated measures raincloud plot']);
