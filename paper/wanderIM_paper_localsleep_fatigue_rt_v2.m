...
res_table.stimulus = dataArray{:, 79};
res_table.response = dataArray{:, 80};
...
%%
UniqueSubj=unique(res_table.SubID);
res_table2=[];
for nSub=1:length(UniqueSubj)
   for nPr=1:60
       res_table2((nSub-1)*60+nPr,:)=nanmean(res_mat(res_table.SubID==UniqueSubj(nSub) & res_table.ProbeN==nPr,:),1);
   end
end
...
State_est=[];
fprintf('E:%2.0f/63\n',0)
myS=unique(res_table2.SubID);
for nE=1:63
    fprintf('\b\b\b\b\b\b\b\bE:%2.0f/63\n',nE)

    State_mdl0= fitlme(res_table2,sprintf('%s~1+(1|SubID)',res_table2.Properties.VariableNames{10+nE}));
    State_mdl1= fitlme(res_table2,sprintf('%s~1+State+(1|SubID)',res_table2.Properties.VariableNames{10+nE}));%,GO_table.Properties.VariableNames{10+nE}));
       
    tempcomp=compare(State_mdl0,State_mdl1);
    State_est=[State_est ; [nE double(State_mdl1.Coefficients(2,2)) double(State_mdl1.Coefficients(2,4)) double(State_mdl1.Coefficients(2,6)) double(State_mdl1.Coefficients(3,2)) double(State_mdl1.Coefficients(3,4)) double(State_mdl1.Coefficients(3,6)) tempcomp.AIC(2) tempcomp.pValue(2)]];
...
%% 
Titles={'MWvsON','MBvsON'};
for nP=1:length(Titles)
    figure;
    addpath((path_fieldtrip)); ft_defaults;
    if nP==1
        temp_topo=State_est(:,3);
        temp_pV=State_est(:,4); %temp_topo(temp_pV>=fdr(temp_pV,0.05))=0;
    elseif nP==2
        temp_topo=State_est(:,6);
        temp_pV=State_est(:,7); %temp_topo(temp_pV>=fdr(temp_pV,0.05))=0;
%     elseif nP==3
%         temp_topo=MBvsMW_est(:,3);
%         temp_pV=MBvsMW_est(:,6); %temp_topo(temp_pV>=fdr(temp_pV,0.05))=0;
    end
    % temp_topo(match_str(layout.label,{'TP9','TP10'}))=0;
    simpleTopoPlot_ft(temp_topo, path_PsychFTlayout,'on',[],0,1);
    colorbar; caxis([-1 1]*max(abs(temp_topo)))
...
subplot(1,3,nState)
addpath((path_fieldtrip)); ft_defaults;
temp_topo=nanmedian(table2array(res_table2((res_table2.State)==num2str(nState),[11:73])),1)';
simpleTopoPlot_ft(temp_topo, path_PsychFTlayout,'on',[],0,1);
